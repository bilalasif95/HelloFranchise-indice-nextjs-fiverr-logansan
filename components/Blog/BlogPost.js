import React from 'react';
import Link from 'next/link';

const BlogPost = () => {
  return (
    <>
      <section className='blog-area bg-f9f9f9 ptb-100'>
        <div className='container'>
          <div className='row'>
            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
									<Link href='/single-blog'>
										<a className='d-block'>
											<img src='/images/blog/blog4.jpg' alt='image' />
										</a>
									</Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user1.jpg' alt='image' />
                        <span>
                          <a href='#'>Taylor</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 7, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        10 Types of Social Proof & What Makes Them Effective
                      </a>
                    </Link>
                  </h3>

                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
                  <Link href='/single-blog'>
                    <a className='d-block'>
                      <img src='/images/blog/blog5.jpg' alt='image' />
                    </a>
                  </Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user2.jpg' alt='image' />
                        <span>
                          <a href='#'>Sarah</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 6, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        Tech Products That Make It Easier to Stay Home
                      </a>
                    </Link>
                  </h3>
                  
                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
                  <Link href='/single-blog'>
                    <a className='d-block'>
                      <img src='/images/blog/blog6.jpg' alt='image' />
                    </a>
                  </Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user3.jpg' alt='image' />
                        <span>
                          <a href='#'>Andy</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 5, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        13 Feel-Good Restaurant Stories from the Pandemic
                      </a>
                    </Link>
                  </h3>

                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
                  <Link href='/single-blog'>
                    <a className='d-block'>
                      <img src='/images/blog/blog7.jpg' alt='image' />
                    </a>
                  </Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user1.jpg' alt='image' />
                        <span>
                          <a href='#'>Taylor</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 4, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        5 Ways to Convert Customers Into Advocates Brand
                      </a>
                    </Link>
                  </h3>
                  
                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
                  <Link href='/single-blog'>
                    <a className='d-block'>
                      <img src='/images/blog/blog8.jpg' alt='image' />
                    </a>
                  </Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user2.jpg' alt='image' />
                        <span>
                          <a href='#'>Sarah</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 3, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        HelloFranchise Tips To-Go: Getting Started With Analytics
                      </a>
                    </Link>
                  </h3>
                  
                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-6 col-md-6'>
              <div className='single-blog-post'>
                <div className='post-image'>
                  <Link href='/single-blog'>
                    <a className='d-block'>
                      <img src='/images/blog/blog9.jpg' alt='image' />
                    </a>
                  </Link>
                </div>

                <div className='post-content'>
                  <ul className='post-meta d-flex align-items-center'>
                    <li>
                      <div className='d-flex align-items-center'>
                        <img src='/images/user3.jpg' alt='image' />
                        <span>
                          <a href='#'>Andy</a>
                        </span>
                      </div>
                    </li>
                    <li>
                      <i className='flaticon-calendar'></i> Aug 2, 2023
                    </li>
                  </ul>

                  <h3>
                    <Link href='/single-blog'>
                      <a>
                        How to Beat the High Cost of Customer Questions
                      </a>
                    </Link>
                  </h3>
                  
                  <Link href='/single-blog'>
                    <a className='link-btn'>
                      <i className='flaticon-right-arrow'></i>
                    </a>
                  </Link>
                </div>
              </div>
            </div>

            <div className='col-lg-12 col-md-12'>
              <div className='pagination-area text-center'>
                <a href='#' className='prev page-numbers'>
                  <i className='bx bx-chevrons-left'></i>
                </a>
                <span className='page-numbers current' aria-current='page'>
                  1
                </span>
                <a href='#' className='page-numbers'>
                  2
                </a>
                <a href='#' className='page-numbers'>
                  3
                </a>
                <a href='#' className='page-numbers'>
                  4
                </a>
                <a href='#' className='next page-numbers'>
                  <i className='bx bx-chevrons-right'></i>
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  )
}

export default BlogPost;